...
class PolynomialFittingLayout(QWidget):
    ...
    def setupTableFitting(self, dialog):
        ...
        self.table_widget.setHorizontalHeaderLabels(['Min. λ (nm)', 'Max. λ (nm)', 'Order', 'Axis', 'Descriptor', 'Transform', 'Auto', 'Label', 'Color'])
    ...
    def populate_table(self, table_widget):
        ...
        for i in range(len(self.num_fitting)):
            ...
            axis_value = QLineEdit()
            axis_value.setText(str(self.axis_value[i])) 
            axis_value.setAlignment(Qt.AlignCenter) 
            axis_value.setValidator(QDoubleValidator(float(self.min_values[i]), float(self.max_values[i]), 2))
            axis_value.textChanged.connect(lambda index, row=i: self.update_axis_value(row, index))
            table_widget.setCellWidget(i, 4, axis_value)
    ...
    def update_axis_value(self, row, index):
        self.axis_value[row] = index
    ...
    def add_row(self, table_widget):
        ...
        self.axis_value.append(self.min_values[0])
        self.name_values.append('')
        self.transform_selected.append(0)
        self.max_values.append(self.max_values[0])
        self.min_values.append(self.min_values[0])
