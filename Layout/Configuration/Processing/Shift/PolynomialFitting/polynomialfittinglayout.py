...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, 
    QHBoxLayout, QPushButton, QSpinBox, QComboBox, QGroupBox, QLineEdit, QTableWidget,
    QColorDialog)

class PolynomialFittingLayout(QWidget):
    ...
    def createPeakFittingButton(self):
        ...
        box = QGroupBox('Fixed λ')
        box.setLayout(vertical_layout)
        return box
    
    def apply(self):
        print('tekannnnnnnnnn')
        self.dialog_open = True

    def show_dialog(self):
        self.dialog = QWidget()
        self.dialog.setWindowTitle("Fixed λ")
        self.setupTableFitting(self.dialog)
        self.dialog.resize(890, 300)
        self.dialog.show()

    def setupTableFitting(self, dialog):
        layout = QVBoxLayout(dialog)
        self.table_widget = QTableWidget(len(self.num_fitting), 8, dialog)
        self.table_widget.horizontalHeader().setStretchLastSection(True)
        self.table_widget.setShowGrid(True)
        self.table_widget.setHorizontalHeaderLabels(['-Δ λ (nm)', '+Δ λ (nm)', 'Order', 'Center (nm)', 'Descriptor', 'Transform', 'Label', 'Color'])
    ...
    def populate_table(self, table_widget):
        ...
        for i in range(len(self.num_fitting)):
            ...
            function_orde.valueChanged.connect(lambda value, row=i: self.update_function_orde(row, value))
            ...
            center_values = QLineEdit()
            center_values.setText(str(self.initial_center_values[i])) 
            center_values.setAlignment(Qt.AlignCenter) 
            center_values.setValidator(QDoubleValidator(float(self.min_values[i]), float(self.max_values[i]), 2))
            center_values.textChanged.connect(lambda index, row=i: self.update_initial_center(row, index))
            table_widget.setCellWidget(i, 4, center_values)
            ...
            table_widget.setCellWidget(i, 6, name)
            ...
            table_widget.setCellWidget(i, 7, color)
    ...
    def update_initial_center(self, row, index):
        self.initial_center_values[row] = index
    ...
    def update_color(self, row, button):
        ...
        button = self.table_widget.cellWidget(row, 7)
    ...
    def add_row(self, table_widget):
        ...
        self.function_orde.append(self.function_orde[0])
