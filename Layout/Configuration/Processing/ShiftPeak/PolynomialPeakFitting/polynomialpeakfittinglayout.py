...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, 
    ...
    QComboBox, QGroupBox, QLineEdit, QTableWidget,
    QColorDialog)

class PolynomialPeakFittingLayout(QWidget):
    ...
    def show_dialog(self):
        self.dialog = QWidget()
        self.dialog.setWindowTitle("Center mass Peak Fitting")
        self.setupTableFitting(self.dialog)
        self.dialog.resize(1180, 300)
        self.dialog.show()

    def setupTableFitting(self, dialog):
        layout = QVBoxLayout(dialog)
        self.table_widget = QTableWidget(len(self.num_fitting), 11, dialog)
        self.table_widget.horizontalHeader().setStretchLastSection(True)
        self.table_widget.setShowGrid(True)
        self.table_widget.setHorizontalHeaderLabels(['-Δ λ (nm)', '+Δ λ (nm)', 'Order', 'Span', 'Init center (nm)', 'Axis', 'Descriptor', 'Transform', 'Auto', 'Label', 'Color'])
    ...
    def populate_table(self, table_widget):
        ...
        for i in range(len(self.num_fitting)):
            min = QLineEdit()
            min.setText(f"{self.min_values[i]}")
            min.setAlignment(Qt.AlignCenter) 
            min.setValidator(QDoubleValidator(float(self.min_values[i]), float(self.max_values[i]), 6))
            min.textChanged.connect(lambda value, row=i: self.update_min(row, value))
            table_widget.setCellWidget(i, 0, min)

            max = QLineEdit()
            max.setText(f"{self.max_values[i]}") 
            max.setAlignment(Qt.AlignCenter) 
            max.setValidator(QDoubleValidator(float(self.min_values[i]), float(self.max_values[i]), 6))
            max.textChanged.connect(lambda value, row=i: self.update_max(row, value))
            table_widget.setCellWidget(i, 1, max)

            function_orde = QSpinBox()
            function_orde.setValue(int(self.function_orde[i])) 
            function_orde.setMinimum(3)
            function_orde.setMaximum(54)
            function_orde.setAlignment(Qt.AlignCenter) 
            function_orde.textChanged.connect(lambda value, row=i: self.update_function_orde(row, value))
            table_widget.setCellWidget(i, 2, function_orde)

            span = QLineEdit()
            span.setText(f"{self.span_values[i]}") 
            span.setAlignment(Qt.AlignCenter) 
            span.setValidator(QDoubleValidator(float(self.min_values[i]), float(self.max_values[i]), 6))
            span.textChanged.connect(lambda value, row=i: self.update_span(row, value))
            table_widget.setCellWidget(i, 3, span)

            initial_center = QLineEdit()
            initial_center.setText(f"{self.initial_center_values[i]}") 
            initial_center.setAlignment(Qt.AlignCenter) 
            initial_center.setValidator(QDoubleValidator(float(self.min_values[i]), float(self.max_values[i]), 6))
            initial_center.textChanged.connect(lambda value, row=i: self.update_initial_center(row, value))
            table_widget.setCellWidget(i, 4, initial_center)

            axis_selected = QComboBox()
            axis_selected.addItems(["X"]) 
            ...
            table_widget.setCellWidget(i, 5, axis_selected)

            method = QComboBox()
            method.addItems(["Center mass"]) 
            ...
            table_widget.setCellWidget(i, 6, method)
            ...
            table_widget.setCellWidget(i, 7, transform)
            ...
            table_widget.setCellWidget(i, 8, inverse)

            name = QLineEdit()
            name.setText(f"{self.name_values[i]}") 
            name.setAlignment(Qt.AlignCenter) 
            name.textChanged.connect(lambda value, row=i: self.update_name(row, value))
            table_widget.setCellWidget(i, 9, name)
            ...
            table_widget.setCellWidget(i, 10, color)
    ...
    def update_span(self, row, value):
        self.span_values[row] = value
    ...
    def update_function_orde(self, row, value):
        self.function_orde[row] = value
    ...
    def update_color(self, row, button):
        ...
        button = self.table_widget.cellWidget(row, 10)
    ...
    def add_row(self, table_widget):
        ...
        self.span_values.append(str(30))
