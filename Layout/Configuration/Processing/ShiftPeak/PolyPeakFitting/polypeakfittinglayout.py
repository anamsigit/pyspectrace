...
from PyQt5.QtWidgets import ( QWidget, QVBoxLayout, 
    QHBoxLayout, QPushButton, QSpinBox, QCheckBox, QComboBox, QGroupBox, QLineEdit, QTableWidget,
    QColorDialog)

class PolyPeakFittingLayout(QWidget):
    ...
    def show_dialog(self):
        self.dialog = QWidget()
        self.dialog.setWindowTitle("Polynomial Peak Fitting")
        self.setupTableFitting(self.dialog)
        self.dialog.resize(1050, 300)
        self.dialog.show()

    def setupTableFitting(self, dialog):
        layout = QVBoxLayout(dialog)
        self.table_widget = QTableWidget(len(self.num_fitting), 10, dialog)
        self.table_widget.horizontalHeader().setStretchLastSection(True)
        self.table_widget.setShowGrid(True)
        self.table_widget.setHorizontalHeaderLabels(['Init center (nm)', 'Min. λ (nm)', 'Max. λ (nm)', 'Order',  'Axis', 'Descriptor', 'Transform', 'Auto', 'Label', 'Color'])
    ...
    def populate_table(self, table_widget):
        ...
        for i in range(len(self.num_fitting)):
            ...
            table_widget.setCellWidget(i, 3, function_orde)
            ...
            table_widget.setCellWidget(i, 4, axis_selected)

            method = QComboBox()
            method.addItems(["Highest peak", "Lowest peak"]) 
            ...
            table_widget.setCellWidget(i, 5, method)
            ...
            table_widget.setCellWidget(i, 6, transform)
            ...
            table_widget.setCellWidget(i, 9, color)
    ...
    def update_color(self, row, button):
        ...
        button = self.table_widget.cellWidget(row, 9)
    ...
    # def release_archive_value(self):
