...
class Spectrometer():
    ...
    def boxcar_correct_spectrum(self, illuminatedSpectrum: list[float], boxcarWidth: int) -> list[float]:
        ...
        @param illuminatedSpectrum[in] the spectrum that will be boxcar corrected.
        @param boxcarWidth[in]         the boxcar width.
        ...
        self.oceandirect.odapi_boxcar_correct_spectrum(self.device_id, err_cp, 
                                                       illuminated_spectrum_array, illuminated_spectrum_array_count,
                                                       boxcarWidth)

        if err_cp[0] != 0:
            error_msg = self.decode_error(err_cp[0],"boxcar_correct_spectrum")
            raise OceanDirectError(err_cp[0], error_msg)

        return list(illuminated_spectrum_array)
    ...
    class Advanced():
        ...
        def get_ip_address_assigned_mode(self) -> bool:
            ...
            @return True if the ip address was statically assigned. False if the ip address was generated via DHCP.
            """
            err_cp = (c_long * 1)(0)
        ...
        def set_ip_address_assigned_mode(self, useStaticIP: bool) -> None:
            """!
            Set the IP address mode to the OBP2 device.
            @param useStaticIP[in] True will use statically assigned IP address. False will use DHCP server for ip assignment. 
            """
            err_cp = (c_long * 1)(0)
            if useStaticIP:
        ...
        def get_network_configuration(self) -> tuple[bool, list[int], list[int], list[int], list[int]]:
            ...
            will return a tuple of 6 objects in this order:
                address mode - True if it's using static IP address otherwise its False.
